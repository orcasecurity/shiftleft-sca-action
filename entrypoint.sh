...
function run_orca_sca_scan() {
  cd "${GITHUB_WORKSPACE}" || exit_with_err "could not find GITHUB_WORKSPACE: ${GITHUB_WORKSPACE}"
  echo "Running Orca SCA scan:"
  echo orca-cli "${GLOBAL_FLAGS[@]}" sca scan "${SCAN_FLAGS[@]}"
  orca-cli "${GLOBAL_FLAGS[@]}" sca scan "${SCAN_FLAGS[@]}"
...
function set_sca_scan_flags() {
  ...
  if [ "${INPUT_NON_STRICT_PACKAGE_DETECTION}" == "true" ]; then
    SCAN_FLAGS+=(--non-strict-package-detection)
  fi
...
annotate() {
  ...
  cp "${OUTPUT_FOR_JSON}/sca.json" "/app/${OUTPUT_FOR_JSON}/" || exit_with_err "error during copy of results"
